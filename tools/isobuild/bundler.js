...
class ServerTarget extends JsImageTarget {
  ...
  async write(builder, {
    ...
    if (format === 'esm') {
      // ESM format: copy only npm-rebuild (needed for postinstall) and esm-loader
      ...
      await builder.write("esm-loader.mjs", {
        file: files.pathJoin(toolsDir, "static-assets", "server", "esm-loader.mjs"),
      });
    } else {
...
To run with Node.js ${process.version}+:
...
Note: DDP (WebSocket) under Bun requires a Bun.serve() bridge on PORT+1.
See the esm-loader documentation for details.
